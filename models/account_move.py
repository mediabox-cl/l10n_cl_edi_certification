...
class AccountMove(models.Model):
    ...
    def _check_document_types_post(self):
        """Override para saltar validación de tipos de documento en certificación"""
        _logger.info(f"Validando tipos de documento - contexto bypass: {self.env.context.get('l10n_cl_edi_certification_bypass')}")
        ...
        if (self.env.context.get('l10n_cl_edi_certification_bypass') or 
            ...
            _logger.info("✓ Saltando validación de tipos de documento para certificación")
            return
            
        # Llamar validación normal para documentos no relacionados con certificación
        _logger.info("Ejecutando validación normal de tipos de documento")
        super()._check_document_types_post()